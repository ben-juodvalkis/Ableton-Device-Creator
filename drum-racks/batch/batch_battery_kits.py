#!/usr/bin/env python3
"""
Batch Battery Kit Processor

Processes all .nbkt files in a Battery Kits folder and creates corresponding
Ableton drum racks with TWO kits per rack (pads 1-16 and 17-32).

Usage:
    python3 batch_battery_kits.py <template.adg> <battery_kits_folder> [--output-folder <path>]

Example:
    python3 batch_battery_kits.py \
...
# Import the kit processors
from main_battery_kit import create_dual_kit_rack
...
def process_all_kits(template_path: Path,
    ...
    """
    Process all .nbkt files in the Battery Kits folder, creating dual-kit racks.
    ...
    print(f"Creating {len(nbkt_files) // 2} dual-kit drum racks (2 kits per rack)")
    print(f"=" * 60)
    ...
    for i in range(0, len(nbkt_files), 2):
        ...
        if i + 1 < len(nbkt_files):
            ...
            try:
                create_dual_kit_rack(template_path, kit_1_path, kit_2_path, output_folder)
                successful += 1
    ...
    print(f"✓ Successful dual racks: {successful}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch process Battery Kit (.nbkt) files to Ableton drum racks',
